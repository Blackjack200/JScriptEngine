...
public class QuercusAdapter extends PSREngineAdapter<QuercusScriptEngine> {
		...
		try {
			...
			this.engine.getQuercus().getModuleContext().addModule("JSE_Extern", new AbstractQuercusModule() {
				public Object extern(Env env,
				                     String className) {
					Object object = JavaModule.java_class(env, className);
	...
	public void invoke(String name, Object... args) {
		AbstractFunction function = this.env.findFunction((StringValue) StringValue.create(name));
		if (function != null) {
			Value[] values = new Value[args.length];
			for (int i = 0; i < args.length; i++) {
				values[i] = this.env.wrapJava(args[i]);
			}
			function.call(this.env, values);
		}
	}
}
