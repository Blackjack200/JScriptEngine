...
public class WalkerRouteFinder extends SimpleRouteFinder {
    ...
    public WalkerRouteFinder(Entity entity) {
        super(entity);
        this.level = entity.getLevel();
    }

    public WalkerRouteFinder(Entity entity, Vector3 start) {
        super(entity);
        this.level = entity.getLevel();
        this.start = start;
    }

    public WalkerRouteFinder(Entity entity, Vector3 start, Vector3 destination) {
        super(entity);
        this.level = entity.getLevel();
        this.start = start;
    ...
    private Block getHighestUnder(Vector3 vector3, int limit) {
        if (limit > 0) {
            for (int y = vector3.getFloorY(); y >= vector3.getFloorY() - limit; y--) {
                ...
                if (isWalkable(block) && canPassThroughBlock(level.getBlock(block.add(0, 1, 0), false))) return block;
            }
            return null;
        }
    ...
    private boolean isWalkable(Vector3 vector3) {
        ...
        return !canPassThroughBlock(block) && canWalkOn(block);
    }

    private boolean isPassable(Vector3 vector3) {
        //TODO: Support Water Path
//        if(Position.fromObject(vector3.add(0,-1,0),entity.getLevel()).getLevelBlock().getId()== BlockID.WATER
//        || Position.fromObject(vector3.add(0,-1,0),entity.getLevel()).getLevelBlock().getId()== BlockID.STILL_WATER){
//            return true;
//        }
    ...
    private boolean hasBlocksAround(ArrayList<Vector3> list) {
        ...
        for (Vector3 vector3 : list) {
            ...
            if (xIsInt && zIsInt) {
                if (canPassThroughBlock(level.getBlock(new Vector3(vector3.getX(), vector3.getY() - 1, vector3.getZ()), false)) ||
                        canPassThroughBlock(level.getBlock(new Vector3(vector3.getX() - 1, vector3.getY() - 1, vector3.getZ()), false)) ||
                        canPassThroughBlock(level.getBlock(new Vector3(vector3.getX() - 1, vector3.getY() - 1, vector3.getZ() - 1), false)) ||
                        canPassThroughBlock(level.getBlock(new Vector3(vector3.getX(), vector3.getY() - 1, vector3.getZ() - 1), false)))
                    return true;
            } else if (xIsInt) {
                if (canPassThroughBlock(level.getBlock(new Vector3(vector3.getX(), vector3.getY() - 1, vector3.getZ()), false)) ||
                        canPassThroughBlock(level.getBlock(new Vector3(vector3.getX() - 1, vector3.getY() - 1, vector3.getZ()), false)))
